...
class Pusher(object):
    def __init__(
            self, app_id=None, key=None, secret=None, host=None, port=None):
        _globals = globals()
...
class Channel(object):
    ...
    def signed_query(self, event, json_data, socket_id):
        ...
        signature = hmac.new(
            self.pusher.secret, string_to_sign, sha_constructor).hexdigest()
...
class TornadoChannel(Channel):
    ...
    def send_request(self, signed_path, data_string):
        ...
        request = tornado.httpclient.HTTPRequest(
            absolute_url, method='POST', body=data_string)
